...
### Challenge
In Blog, enumerate username(s) and then brute force the password to the account. Hinted by the fact that the test:test username and password combo told me that the username was invalid. 

Used intruder to find the username info then ran it again to find the password and used the follow redirects function. 
